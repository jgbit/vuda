...
namespace vuda
{
                                                ...
                                                const uint32_t stream)
    {
        ...
        //static std::atomic_flag creation_lock = ATOMIC_FLAG_INIT;
        static std::atomic<bool> creation_lock = false;
        ...
        while(true)
        {
            ...
            if(creation_lock.exchange(true) == false)
            {
                ...
                creation_lock.store(false);
            }
            else
            {
                //
                // wait for creation proccess to complete and try again
                while(creation_lock.load() == true)
                    ;
            }
        }
    }
